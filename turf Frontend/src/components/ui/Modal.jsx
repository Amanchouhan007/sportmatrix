...
export default function Modal({ isOpen, onClose, title, children, size = 'md' }) {
    ...
    const modalContent = (
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-3 sm:p-6 md:p-8">
            ...
            >
                ...
                <div className="flex items-center justify-between px-4 sm:px-8 py-3.5 sm:py-6 border-b border-slate-200/80 shrink-0 bg-slate-50/50">
                    <h3 className="text-base sm:text-xl md:text-2xl font-black text-slate-900 tracking-tight leading-snug pr-3">
                        {title}
                    </h3>
                    <button
                        onClick={onClose}
                        className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-900 transition-all duration-200 flex items-center justify-center cursor-pointer shrink-0 border border-slate-200/60"
