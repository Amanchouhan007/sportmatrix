...
import {
    ...
    HiLocationMarker, HiChevronRight, HiEye, HiX, HiFire
} from 'react-icons/hi'
...
export default function PlayerLeaderboardPage() {
    ...
    const handleApproveHandshake = (matchId, tier) => {
        const approvedEntry = {
            ...
            name: 'Rahul Sharma (Vijay Nagar)',
            avatar: '🏏',
            team: 'Vijay Nagar Blasters (Indore)',
            city: 'Indore',
            sport: 'Cricket',
    ...
    const handleFormSubmit = (e) => {
        ...
        if (!formScore.playerName.trim()) {
            addToast('Please enter player name', 'error')
            return
        }
        ...
        addToast(`🎉 Score Verified & Certified! ${newPlayerEntry.name} ranked in Indore Leaderboard with ${formScore.tier}!`, 'success')
    }

    const fillDemoScore = () => {
        setFormScore({
            playerName: 'Aman Patel',
            teamName: 'Palasia Super Strikers',
            zone: 'Palasia',
            role: 'Top-Order Batsman',
            tier: 'Tier 2',
            runs: 94,
        ...
        addToast('⚡ Demo match scorecard filled! Click "Submit & Certify" below.', 'info')
    }
    ...
    return (
        <div className="min-h-screen bg-white text-[#111827] pt-20 pb-20 px-4 sm:px-6 lg:px-8 animate-in fade-in duration-300">
            {/* Ambient Background Glow */}
            <div className="max-w-7xl mx-auto space-y-6">
                
                {/* ═══════════════════════════════════════════════════
                    HERO HEADER & VERIFIED ENGINE INTRO
                ═══════════════════════════════════════════════════ */}
                <div className="relative rounded-3xl bg-gradient-to-br from-emerald-50/80 via-white to-slate-50 border border-[#E5E7EB] p-6 sm:p-10 shadow-sm overflow-hidden text-center space-y-4">
                    <div className="absolute -top-24 -left-24 w-72 h-72 bg-[#16A34A]/5 rounded-full blur-3xl pointer-events-none" />
                    <div className="absolute -bottom-24 -right-24 w-72 h-72 bg-[#C8FF2E]/15 rounded-full blur-3xl pointer-events-none" />

                    <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-emerald-100/70 border border-emerald-300 text-[#065F46] text-xs font-black uppercase tracking-widest shadow-2xs">
                        <span>🏆</span>
                        <span>4-Tier Verified Performance Engine · Indore Chapter</span>
                    </div>

                    <h1 className="text-3xl sm:text-5xl font-black tracking-tight text-[#111827] uppercase italic">
                        Indore Turf <span className="text-[#16A34A]">Player Leaderboards</span>
                    </h1>

                    <p className="text-slate-600 text-xs sm:text-sm font-semibold max-w-2xl mx-auto leading-relaxed">
                        Rankings computed live across Indore turfs. Self-reported scores carry 0.0x weight, Captain Handshakes carry 1.0x, Platform Umpire Matches carry 1.5x, and Official Tournaments carry 2.0x weight.
                    </p>

                    {/* Tier Multiplier Pill Badges */}
                    <div className="flex flex-wrap items-center justify-center gap-2 pt-2 text-xs">
                        <span className="px-3 py-1 rounded-xl bg-slate-100 text-slate-600 border border-slate-200 font-bold">
                            Tier 0: Self-Entry <strong className="text-slate-500 font-mono">(0.0x)</strong>
                        </span>
                        <span className="px-3 py-1 rounded-xl bg-blue-50 text-blue-700 border border-blue-200 font-bold">
                            Tier 1: Captain Handshake <strong className="text-blue-600 font-mono">(1.0x)</strong>
                        </span>
                        <span className="px-3 py-1 rounded-xl bg-emerald-50 text-[#065F46] border border-emerald-300 font-black shadow-2xs">
                            Tier 2: Paid Umpire <strong className="text-emerald-700 font-mono">(1.5x ⚖️)</strong>
                        </span>
                        <span className="px-3 py-1 rounded-xl bg-purple-50 text-purple-700 border border-purple-200 font-black shadow-2xs">
                            Tier 3: Official Tournament <strong className="text-purple-600 font-mono">(2.0x 🏆)</strong>
                        </span>
                    </div>
                </div>
                ...
                {top3.length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pt-2">
                        {/* 🥈 #2 Silver */}
                        {top3[1] && (
                            <div className="order-2 md:order-1 bg-gradient-to-b from-slate-50 via-white to-white rounded-3xl border-2 border-slate-200 p-6 flex flex-col justify-between shadow-sm hover:shadow-md relative overflow-hidden text-center space-y-4 hover:border-slate-300 transition-all">
                        ...
                        {top3[0] && (
                            <div className="order-1 md:order-2 bg-gradient-to-b from-amber-50/80 via-white to-amber-50/30 rounded-3xl border-2 border-amber-400 p-6 flex flex-col justify-between shadow-lg relative overflow-hidden text-center space-y-4 scale-100 md:scale-105 z-10">
                                ...
                                <span className="absolute top-4 right-4 text-xs font-black uppercase bg-gradient-to-r from-amber-400 to-yellow-500 text-slate-950 px-3 py-0.5 rounded-full shadow-sm">
                                    👑 GOLD CHAMPION
                                </span>
                        ...
                        {/* 🥉 #3 Bronze */}
                        {top3[2] && (
                            <div className="order-3 bg-gradient-to-b from-orange-50/60 via-white to-white rounded-3xl border-2 border-amber-200 p-6 flex flex-col justify-between shadow-sm hover:shadow-md relative overflow-hidden text-center space-y-4 hover:border-amber-300 transition-all">
                ...
                <div className="bg-white rounded-3xl border border-[#E5E7EB] overflow-hidden shadow-sm">
                    <div className="p-5 border-b border-slate-100 flex items-center justify-between">
                        <h3 className="font-black text-base text-[#111827] flex items-center gap-2">
                            <span>📊</span> Official Standings Table ({rankedPlayers.length} Ranked)
                        </h3>
                        ...
                        <table className="w-full text-left text-xs">
                            ...
                            <tbody className="divide-y divide-slate-100 font-medium">
                                {rankedPlayers.map((player, idx) => (
                                    <tr key={player.id} className="hover:bg-emerald-50/30 transition-colors">
                                        ...
                                        <td className="py-3.5 px-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="font-black text-[#111827] text-sm">{player.name}</div>
